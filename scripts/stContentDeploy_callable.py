...
def get_App_Info(v_env_name, v_app_name):
	...
	if REC_FOUND == 0:
		...
		if debug > 0:
			print ( ln)
		#tk.messagebox.showinfo("Error","No Record Found in file "+env_app_file)
		write_to_file(fh_out, ln)
		write_to_file(fh_err_out, ln)
		sys.exit(1)
...
def backup_app(*args):
	...
	if len(os.listdir(app_destination)) > 0:
		...
		ln= "\nNo existing content there, so no backup"
		write_to_file(fh_out, ln)
	...
	if (ffname.endswith(".tar")) or (ffname.endswith(".tar.gz")) :
		...
		for tarinfo in tar:
			...
			if ((re.search("httpd.ini", aa,re.IGNORECASE)) or (re.search("web.conf", aa,re.IGNORECASE))) :
				if debug > 0:
					print  (aa)
				ln= "\n It has httpd.ini and web.conf , removed "
				write_to_file(fh_out, ln)
				...
				try:
					...
					ln="\nFailed with:"+ e # look what it says
					write_to_file(fh_out, ln)
					write_to_file(fh_err_out, ln)
					sys.exit(1)
...
def ziptar_is_valid(arc_file_name): 
	...
	print("\nArchive pakg file:  "+arc_file_name)
...
def extract_file_to_app(ffile_name):	#ffile_name is package name either zip/tar/gz
	...
	if ziptar_is_valid(ffile_name) > 0:
		ln="\nInvalid package/file "+ffile_name+"....."
		if debug > 0:
			print(ln)
	...
	if ext == ".tar" or ext == ".gz" :
		...
		#ln="Error: No deployment package/file or Incorrect deployment package found.\nDeployment Failed....."
...
if 	int(DEPLOYMENT_STATUS) > 0:
	ln = '\nDeployemnt Failed. For details, Please check logs  '+log_file_name
	write_to_file(fh_out, ln)
	write_to_file(fh_err_out, ln)
else:
